...
Configuration Steps
Assigned static IP addresses to PC1 and PC2 on their respective subnets.
Configured the router's G0/0 and G0/1 interfaces with the gateway IP for each subnet.
Brought up router interfaces with no shutdown.
Verified switches were passing traffic between connected devices (Layer 2, no VLAN config needed for this basic lab).
Verification
